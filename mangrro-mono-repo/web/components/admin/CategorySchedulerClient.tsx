...
export default function CategorySchedulerClient({
  ...
  const categoryOptions = initialCategories.filter(
    (category) => category.categoryType === "category"
  );
  const categoryIdSet = new Set(categoryOptions.map((category) => category.id));
  const initialSelectedIds = initialSelection.ids.filter((id) =>
  ...
  const [schedules, setSchedules] = useState<
    Record<string, CategorySchedule>
  >({});
  ...
  const loadCategories = async () => {
    ...
    const loaded = (json.data ?? json ?? []).filter(
      (category: AdminCategory) => category.categoryType === "category"
    );
    setCategories(loaded);
    const loadedIdSet = new Set(loaded.map((category) => category.id));
    setSelectedCategories((prev) => prev.filter((id) => loadedIdSet.has(id)));
    setActiveCategoryId((prev) =>
      prev && loadedIdSet.has(prev) ? prev : null
    );
  };

  const loadSchedules = async () => {
    try {
      const res = await fetch("/api/admin/categoryscheduler");
      const json = await res.json();
      const arr: any[] = json.data ?? json ?? [];

      const mapped: Record<string, CategorySchedule> = {};
      for (const raw of arr) {
        if (!raw.categoryId) continue;

        mapped[raw.categoryId] = {
          id: raw.id,                      
  ...
  const deleteSchedule = async (categoryId: string) => {
    ...
    try {
      await fetch("/api/admin/categoryscheduler", {
        method: "DELETE",
  ...
  const applyToAll = () => {
    ...
    setSchedules(prev => {
      ...
      const monday = current.timeslots["Monday"]; // Source row
  ...
  return (
    <AdminShell>
      ...
      <AdminCard
        ...
        description="Pick which categories to schedule"
      >
        <AdminTable
          columns={columns}
          data={categories}
      ...
      <AdminCard
        ...
        description={
          activeCategory
            ...
            : "Select a category above to edit schedule"
        }
      >
        {!activeCategoryId ? (
          <p className="text-sm text-slate-500">
            No category selected. Choose one above.
          </p>
        ) : (
