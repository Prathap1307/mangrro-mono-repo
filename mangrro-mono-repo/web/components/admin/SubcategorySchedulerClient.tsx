...
export default function SubcategorySchedulerClient({
  initialCategories,
  initialSelection,
}: Props) {
  ...
  const [schedules, setSchedules] = useState<
    Record<string, SubcategorySchedule>
  >({});
  ...
  const loadSchedules = async () => {
    try {
      const res = await fetch("/api/admin/subcategory-scheduler");
      const json = await res.json();
      const arr: any[] = json.data ?? json ?? [];

      const mapped: Record<string, SubcategorySchedule> = {};
      for (const raw of arr) {
        if (!raw.subcategoryId) continue;

        mapped[raw.subcategoryId] = {
          id: raw.id,
  ...
  const deleteSchedule = async (categoryId: string) => {
    ...
    try {
      await fetch("/api/admin/subcategory-scheduler", {
        method: "DELETE",
  ...
  const handleSave = async () => {
    try {
      ...
      await fetch("/api/admin/settings/subcategory-schedule", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <AdminShell>
      <AdminPageTitle
        title="Subcategory Scheduler"
        description="Choose subcategories and apply weekly opening windows."
        action={
      ...
      <AdminCard
        ...
        description="Pick which subcategories to schedule"
      >
        <AdminTable
          columns={columns}
          data={categories}
      ...
      <AdminCard
        ...
        description={
          activeCategory
            ...
            : "Select a subcategory above to edit schedule"
        }
      >
        {!activeCategoryId ? (
          <p className="text-sm text-slate-500">
            No subcategory selected. Choose one above.
          </p>
        ) : (
                  ...
                  <div className="mb-2 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        Applies to the selected subcategory
                      </p>
                    </div>
